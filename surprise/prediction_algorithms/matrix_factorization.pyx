...
class SVD_ReTrain(AlgoBase):
    ...
    def fit(self, trainset, prev_algo=None, new_trainset=None):
        '''prev_algo is a trained MF algorithm new_data is a list of length-3 tuples'''
        AlgoBase.fit(self, trainset)
        self.sgd(trainset, pre_algo, new_trainset)

        return self

    def sgd(self, trainset, prev_algo, new_trainset):
        ...
        if prev_algo:
            self.bu = prev_algo.bu
            self.bi = prev_algo.bi
            self.pu = prev_algo.pu
            self.qi = prev_algo.qi
        ...
        if new_trainset:
            ...
            pu = np.asarray(pu)
            qi = np.asarray(qi)
            self.bu = np.concatenate(self.bu, bu)
